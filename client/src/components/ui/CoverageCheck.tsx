...
const HQ = { lat: 43.5712, lng: -70.7287, name: "North Waterboro" };
const MAX_MILES = 60;
...
function findMatch(raw: string): MatchResult | null {
  ...
  if (/^\d{5}$/.test(q)) {
    ...
    if (byZip) {
      const miles = milesBetween(HQ, byZip);
  ...
  const miles = milesBetween(HQ, found);
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1 text-[11px] sm:text-xs text-muted-foreground">
                    <span className="flex items-center gap-1">
                      ...
                      {result.miles.toFixed(1)} mi from HQ
                    </span>
                    {result.served && (
                      <span className="flex items-center gap-1">
